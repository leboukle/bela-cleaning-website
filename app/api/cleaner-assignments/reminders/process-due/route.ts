...
// Accepted assignments whose booking looks due for the 72-hour cleaner
// reminder. Completely separate from /api/reminders/process-due (the
// customer reminder) — never touches that sheet's reminder columns.
// Re-validates due-ness independently — see cleanerReminderService.ts.
import { NextResponse } from "next/server";
import { verifySchedulerRequest, SchedulerAuthError } from "@/lib/booking/server/schedulerAuth";
import { processCleanerReminderForAssignment } from "@/lib/booking/server/cleanerReminderService";
...
export async function POST(request: Request) {
  ...
  for (const assignmentId of assignmentIds) {
    try {
      const outcome = await processCleanerReminderForAssignment(assignmentId, bookingRepository, assignmentRepository, notifications, now);
      results.push(outcome);
    } catch (error) {
      console.error(
        `[api/cleaner-assignments/reminders/process-due] unhandled error: assignmentId=${assignmentId}`,
