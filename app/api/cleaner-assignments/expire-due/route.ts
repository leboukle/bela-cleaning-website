...
// cleaner-assignment scheduler (see apps-script/cleanerAssignmentScheduler.gs),
// on an hourly time-driven trigger. Apps Script determines which
// assignment rows look past their Response Deadline by reading the
// Cleaner Assignments sheet directly; this route re-reads each
// assignment's authoritative state independently and never trusts
// due-ness asserted by the caller — see assignmentService.ts's
// processExpireDueAssignment. Authenticated via the same shared-secret
// header the payment/reminder schedulers already use.
import { NextResponse } from "next/server";
import { verifySchedulerRequest, SchedulerAuthError } from "@/lib/booking/server/schedulerAuth";
import { processExpireDueAssignment } from "@/lib/booking/server/assignmentService";
...
export async function POST(request: Request) {
  ...
  const now = new Date();
  const results = [];
  // Sequential, not parallel — same rationale as the payment/reminder
  // scheduler routes: keeps Sheets API usage within quota and avoids
  // interleaving one assignment's read-then-write with another's.
  for (const assignmentId of assignmentIds) {
    try {
      const outcome = await processExpireDueAssignment(assignmentId, bookingRepository, assignmentRepository, notifications, now);
      results.push(outcome);
    } catch (error) {
      console.error(`[api/cleaner-assignments/expire-due] unhandled error: assignmentId=${assignmentId}`, error instanceof Error ? error.message : "unknown error");
