...
import { arrivalWindowStartSpec } from "./serviceTime";
...
describe("calculateScheduledChargeAt", () => {
  it("computes start + duration + 1 hour for a morning window in EST (winter, UTC-5)", () => {
    ...
    const result = calculateScheduledChargeAt("2026-01-15", arrivalWindowStartSpec("morning"), 240, TZ);
  ...
  it("computes start + duration + 1 hour for an afternoon window in EDT (summer, UTC-4)", () => {
    ...
    const result = calculateScheduledChargeAt("2026-07-15", arrivalWindowStartSpec("afternoon"), 180, TZ);
  ...
  it("uses the correct UTC offset on the day DST springs forward (2026-03-08)", () => {
    ...
    const result = calculateScheduledChargeAt("2026-03-08", arrivalWindowStartSpec("midday"), 120, TZ);
  ...
  it("uses the correct UTC offset on the day DST falls back (2026-11-01)", () => {
    ...
    const result = calculateScheduledChargeAt("2026-11-01", arrivalWindowStartSpec("early-afternoon"), 90, TZ);
    expect(result.toISOString()).toBe("2026-11-01T19:30:00.000Z");
  });

  it("maps each legacy arrival window to its documented start time", () => {
    const morning = calculateScheduledChargeAt("2026-06-01", arrivalWindowStartSpec("morning"), 0, TZ);
    const midday = calculateScheduledChargeAt("2026-06-01", arrivalWindowStartSpec("midday"), 0, TZ);
    const earlyAfternoon = calculateScheduledChargeAt("2026-06-01", arrivalWindowStartSpec("early-afternoon"), 0, TZ);
    const afternoon = calculateScheduledChargeAt("2026-06-01", arrivalWindowStartSpec("afternoon"), 0, TZ);
  ...
  it("Milestone 6 amendment: computes start + duration + 1 hour for an exact Service Start Time", () => {
    // 9:00 AM local, 3.5-hour (210 min) duration -> ends 12:30 PM, +1h -> 1:30 PM EDT = 17:30 UTC.
    const result = calculateScheduledChargeAt("2026-07-15", { hour: 9, minute: 0 }, 210, TZ);
